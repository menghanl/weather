/**
 * wx — terminal variant.
 * Dual-unit weather: current conditions + 10-day forecast, °F and °C together.
 * Data: Open-Meteo forecast + geocoding (no key), BigDataCloud reverse geocode.
 */

/* WMO code → METAR-ish short code + plain label */
const WMO = {
  0: { code: "CLR", label: "Clear" },
  1: { code: "FEW", label: "Mainly clear" },
  2: { code: "SCT", label: "Partly cloudy" },
  3: { code: "OVC", label: "Overcast" },
  45: { code: "FG", label: "Fog" },
  48: { code: "FZFG", label: "Icy fog" },
  51: { code: "-DZ", label: "Light drizzle" },
  53: { code: "DZ", label: "Drizzle" },
  55: { code: "+DZ", label: "Heavy drizzle" },
  56: { code: "FZDZ", label: "Freezing drizzle" },
  57: { code: "FZDZ", label: "Freezing drizzle" },
  61: { code: "-RA", label: "Light rain" },
  63: { code: "RA", label: "Rain" },
  65: { code: "+RA", label: "Heavy rain" },
  66: { code: "FZRA", label: "Freezing rain" },
  67: { code: "FZRA", label: "Freezing rain" },
  71: { code: "-SN", label: "Light snow" },
  73: { code: "SN", label: "Snow" },
  75: { code: "+SN", label: "Heavy snow" },
  77: { code: "SG", label: "Snow grains" },
  80: { code: "-SH", label: "Light showers" },
  81: { code: "SH", label: "Showers" },
  82: { code: "+SH", label: "Heavy showers" },
  85: { code: "SHSN", label: "Snow showers" },
  86: { code: "SHSN", label: "Heavy snow showers" },
  95: { code: "TS", label: "Thunderstorm" },
  96: { code: "TSGR", label: "Thunderstorm + hail" },
  99: { code: "TSGR", label: "Thunderstorm + hail" },
};
...
function renderPopup(results, { loading = false, empty = false } = {}) {
  ...
  if (loading) {
    els.popup.innerHTML = `<li class="search-popup__loading" role="presentation">querying geocoder </li>`;
  } else if (empty) {
    els.popup.innerHTML = `<li class="search-popup__empty" role="presentation">-- no match --</li>`;
  } else {
...
function wmo(code) {
  return WMO[code] ?? { code: "UNK", label: "Unknown" };
}

/** Cold cyan → phosphor green → amber, mapped over roughly -10 °C … 35 °C. */
...
function dayFull(isoDate, index) {
...
function bigTempHtml(c, feelsC, unit) {
  const conv = (v) => (unit === "F" ? round(cToF(v)) : round(v));
  const feels = Number.isFinite(feelsC)
    ? `<span class="temp__feels">feels ${conv(feelsC)}&deg;</span>`
    : "";
  return `
    <div class="temp temp--${unit.toLowerCase()}">
      <span class="temp__unit">&deg;${unit}</span>
      <span class="temp__value">${conv(c)}<span class="temp__deg">&deg;</span></span>
      ${feels}
    </div>`;
}

function hlRowHtml(key, c) {
  return `
    <span class="hl">
      <span class="hl__key">${key}</span>
      <span class="hl__f">${round(cToF(c))}&deg;F</span>
      <span class="hl__c">${round(c)}&deg;C</span>
    </span>`;
}

function renderCurrent(placeName, data) {
  const cur = data.current;
  const { code, label } = wmo(cur.weather_code);
  ...
  els.temps.innerHTML =
    bigTempHtml(cur.temperature_2m, cur.apparent_temperature, "F") +
    `<span class="temps__divider" aria-hidden="true"></span>` +
    bigTempHtml(cur.temperature_2m, cur.apparent_temperature, "C");
  els.temps.setAttribute(
    "aria-label",
    `Now ${f} degrees Fahrenheit, ${c} degrees Celsius`
  );

  els.condition.innerHTML =
    `<span class="cond__code" aria-hidden="true">${escapeHtml(code)}</span>` +
    escapeHtml(label);

  els.hl.innerHTML = hlRowHtml("HI", hi) + hlRowHtml("LO", lo);
  els.hl.setAttribute(
    "aria-label",
...
function renderDaily(data) {
  ...
  els.dailyList.innerHTML = time
    .map((iso, i) => {
      ...
      const { code, label } = wmo(weather_code[i]);
      ...
      return `
        <li class="day${i === 0 ? " day--today" : ""}">
          <span class="day__name" title="${escapeHtml(dayFull(iso, i))}"><span aria-hidden="true">${dayLabel(
            iso,
            i
          )}</span><span class="sr-only">${escapeHtml(dayFull(iso, i))}</span></span>
          <span class="day__code" title="${escapeHtml(
            label
          )}"><span aria-hidden="true">${escapeHtml(
            code
          )}</span><span class="sr-only">${escapeHtml(label)}</span></span>
          <span class="t t--lo">
            <span class="sr-only">Low </span>
            <span class="t__f">${round(cToF(lo))}&deg;<span class="sr-only">F</span></span>
            <span class="t__c">${round(lo)}&deg;C</span>
          </span>
          ...
          <span class="t t--hi">
            ...
            <span class="t__f">${round(cToF(hi))}&deg;<span class="sr-only">F</span></span>
            <span class="t__c">${round(hi)}&deg;C</span>
          </span>
        </li>`;
    })
    .join("");
...
async function loadFromCoords(lat, lon) {
  setView("loading");
  setStatus("resolving coordinates…", "loading");
...
function useGeolocation() {
  ...
  setView("loading");
  setStatus("acquiring gps fix…", "loading");
...
// Boot: last place, else geolocation, else empty state
