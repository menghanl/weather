/**
 * Swiss Minimal variant — dual-unit weather: current conditions + 10-day forecast.
 * Same behaviour as the main app; monochrome typographic presentation.
 * Data: Open-Meteo forecast + geocoding (no API key), BigDataCloud reverse geocode.
 *
 * Sky "icons" are abstract monochrome marks, not emoji, to keep the sheet
 * strictly black / white / gray.
 */

const WMO = {
  0: { label: "Clear", mark: "○" },
  1: { label: "Mainly clear", mark: "◔" },
  2: { label: "Partly cloudy", mark: "◑" },
  3: { label: "Overcast", mark: "●" },
  45: { label: "Fog", mark: "≡" },
  48: { label: "Icy fog", mark: "≡" },
  51: { label: "Light drizzle", mark: "⁚" },
  53: { label: "Drizzle", mark: "⁚" },
  55: { label: "Heavy drizzle", mark: "⁘" },
  56: { label: "Freezing drizzle", mark: "⁚" },
  57: { label: "Freezing drizzle", mark: "⁘" },
  61: { label: "Light rain", mark: "╷" },
  63: { label: "Rain", mark: "╿" },
  65: { label: "Heavy rain", mark: "‖" },
  66: { label: "Freezing rain", mark: "╿" },
  67: { label: "Freezing rain", mark: "‖" },
  71: { label: "Light snow", mark: "·" },
  73: { label: "Snow", mark: "∗" },
  75: { label: "Heavy snow", mark: "✳" },
  77: { label: "Snow grains", mark: "∴" },
  80: { label: "Light showers", mark: "╱" },
  81: { label: "Showers", mark: "╱╱" },
  82: { label: "Heavy showers", mark: "⫽" },
  85: { label: "Snow showers", mark: "∗" },
  86: { label: "Heavy snow showers", mark: "✳" },
  95: { label: "Thunderstorm", mark: "↯" },
  96: { label: "Thunderstorm, hail", mark: "↯" },
  99: { label: "Thunderstorm, hail", mark: "↯" },
};
...
function setActiveIndex(i) {
  const items = els.popup.querySelectorAll(".popup__item");
  if (!items.length) {
    searchState.active = -1;
    els.input.removeAttribute("aria-activedescendant");
    return;
...
function renderPopup(results, { loading = false, empty = false } = {}) {
  ...
  if (loading) {
    els.popup.innerHTML = `<li class="popup__note" role="presentation">Searching</li>`;
  } else if (empty) {
    els.popup.innerHTML = `<li class="popup__note" role="presentation">No cities found</li>`;
  } else {
        ...
        return `
          <li role="presentation">
            <button
              ...
              aria-selected="false"
            >
              <span class="popup__name">${escapeHtml(r.name)}</span>
              ${
                meta
                  ? `<span class="popup__meta">${escapeHtml(meta)}</span>`
                  : ""
              }
            </button>
          </li>`;
...
function wmo(code) {
  return WMO[code] ?? { label: "Unknown", mark: "—" };
}
...
function renderCurrent(placeName, data) {
  ...
  const { label, mark } = wmo(cur.weather_code);
  ...
  els.condition.innerHTML =
    `<span class="cond__code">WMO ${pad2(cur.weather_code)}</span>` +
    `<span class="cond__mark" aria-hidden="true">${mark}</span>` +
    escapeHtml(label);
...
function renderDaily(data) {
  ...
  els.dailyList.innerHTML = time
    .map((iso, i) => {
      ...
      const { mark, label } = wmo(weather_code[i]);
      const left = clamp(((lo - allMin) / span) * 100, 0, 100);
      const width = clamp(((hi - lo) / span) * 100, 2, 100 - left);
      const nowDot =
      ...
      return `
        <li class="day${i === 0 ? " day--today" : ""}">
          <span class="day__no" aria-hidden="true">${pad2(i + 1)}</span>
          <span class="day__name">${dayLabel(iso, i)}</span>
          <span class="day__mark" role="img" aria-label="${escapeHtml(
            label
          )}">${mark}</span>
          <span class="t t--lo">
          ...
          <span class="day__range" aria-hidden="true">
            <span class="bar">
              <span class="bar__fill" style="left:${left.toFixed(
                1
              )}%;width:${width.toFixed(1)}%;animation-delay:${(i * 25).toFixed(
                0
              )}ms"></span>
              ${nowDot}
            </span>
...
els.popup.addEventListener("click", (e) => {
  const btn = e.target.closest(".popup__item");
  if (!btn) return;
