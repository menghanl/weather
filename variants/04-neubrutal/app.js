...
/** Flat poster palette — stepped, no gradients. Cold blue → hot pink. */
const TEMP_STEPS = [
  { max: -2, color: "#9fd4ff" }, // ice
  { max: 6, color: "#2b4fff" }, // electric blue
  { max: 14, color: "#00d3a7" }, // mint
  { max: 22, color: "#ffd400" }, // yellow
  { max: 29, color: "#ff8a1f" }, // orange
  { max: Infinity, color: "#ff3d8b" }, // hot pink
];

function tempColor(c) {
  for (const step of TEMP_STEPS) if (c < step.max) return step.color;
  return "#ff3d8b";
}
...
function bigTempHtml(c, unit) {
  ...
  return `
    <div class="temp temp--${unit.toLowerCase()}">
      <span class="temp__unit">°${unit}</span>
...
function renderCurrent(placeName, data) {
  ...
  els.temps.innerHTML =
    bigTempHtml(cur.temperature_2m, "F") + bigTempHtml(cur.temperature_2m, "C");
  ...
  els.hl.innerHTML = hlChipHtml("HI", hi) + hlChipHtml("LO", lo);
  els.hl.setAttribute(
    "aria-label",
...
function renderDaily(data) {
  ...
  els.dailyList.innerHTML = time
    .map((iso, i) => {
      ...
      const width = clamp(((hi - lo) / span) * 100, 4, 100 - left);
      const nowDot =
